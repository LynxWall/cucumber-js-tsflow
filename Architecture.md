...
- The on-disk stores: the transpile cache (content-addressed), the selective-load index (every skipped file's record is validated against the recorded stamps before it is trusted) and Node's compile cache (keyed on the source).
