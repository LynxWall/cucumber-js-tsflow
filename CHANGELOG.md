...
- `tsnode-service.mjs` (internal; it existed only to build the `ts-node` service the esbuild ESM loaders no longer use), the `getEsmHooks` / `createHookExports` helpers and the `tsNodeHooks` / `getTsNodeHooks` / `handleTsFiles` options of `resolveSpecifier` in `loader-utils.mjs`, and the `getFormat` / `transformSource` exports of `esnode-loader` and `esvue-loader` — legacy hook names that Node has not called since 16.12. `esbuild-transpiler.mjs` and its bundled `esbuild-transpiler-cjs.js` (a ts-node `Transpiler` plugin) are still built and exported but are no longer used by any tsflow loader.
