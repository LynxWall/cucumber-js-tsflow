...
Feature: Support files that fail to load
  ...
  startup progress shuts down, the error names the file and the CLI exits
  with code 1. The "missing-import" profile of each workspace adds a fixture
  whose import cannot be resolved to two working support files.

  Scenario: A support file whose import cannot be resolved fails the run and is named
