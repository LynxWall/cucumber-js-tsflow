...
Feature: Watch mode after a failing run
  ...
  run rather than a crash. These scenarios start the CLI on the
  "missing-import" profile of each workspace, which adds a fixture whose
  import cannot be resolved to two working support files.

  Scenario: A run that fails to load can be rerun and quit cleanly
    Given a watch session on the "missing-import" profile has completed its first run
