...
1. Convert every step and hook in the profile's support globs before the first run. The cucumber-tsflow runner
   cannot execute a step registered with CucumberJS's own `Given()`/`When()`/`Then()` or a hook registered with
   `Before()`/`After()`: when it reaches one, the run aborts with `Unable to find StepBinding!` (exit code `1` in
   serial mode). On a large suite, convert one area at a time behind a profile whose `paths` and support globs
   cover only the converted area, and grow the globs as you go.
