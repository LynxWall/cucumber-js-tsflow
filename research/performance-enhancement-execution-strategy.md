...
- **Stage:** 12c, review refactors, of Phase 12 (the release gate). Group 1 of 6 (the 12b fixes AA, Z, AB, AC,
  AD, AE, AF) is complete with the boundary matrix green; groups 2 to 6 have not started. Group 2 (A, B, C, K/V,
  L) is next and needs no spec changes.
- **Branch:** group 1 is one squashed commit, `a29eb42`, on top of the triage commit `1c842e6`; the split of this
  document into `execution-strategy/` follows it. Both are pushed. The commit workflow (small commits inside a
  group, one squashed commit per group before the hand-off and before any push) is recorded at the end of the
  12c hand-off.
- **First act of the next session:** start group 2 (A, B, C, K/V, L) from the 12c triage table, with the
  per-commit cadence from the 12c stage definition.
...
| [stage-12c-hand-off.md](execution-strategy/stage-12c-hand-off.md) | Review refactors, written as the groups land; group 1 and the commit workflow so far | Continuing 12c |
