...
Added by the 12b failure-path pass (see [Stage 12b hand-off](#stage-12b-hand-off)); each is classified there as a fix for 12c, a test, or closed:
...
**Status: in progress.** Group 1 (the 12b fixes: AA, Z, AB, AC, AD, AE, AF) landed 2026-09-23 as `e601aa4` through `b60a149` with the boundary matrix green; see [Stage 12c hand-off](#stage-12c-hand-off). Groups 2–6 next.
...
[Behavior discovery](#12b-behavior-discovery): the end-to-end scenarios the 12a tests left to the spec matrix, and
...
[Review refactors](#12c-review-refactors): the triaged findings in six groups, one concern per commit. **Group 1
is complete; groups 2–6 have not started.**

### State of the tree

- Branch `2026-09-speed-enhancements`, started from `1c842e6` (the triage commit). Group 1 is seven commits, one
  per finding, in the order the plan gave: `e601aa4` (AA), `731f126` (Z), `e35688d` (AB), `846b4a1` (AC), `452c529`
  (AD), `b480a60` (AE), `b60a149` (AF). The tree was clean after each; the cadence held after every commit (`yarn
...
- On `b60a149`: `yarn build` clean, no stray `.js` under `src/`; `yarn test:unit` green, **238 tests** (218 at the
...
- **Real-console verification** (`verify-console-output`, preflight green: `isTTY=true`, glyphs single-cell,
  worker TTY stream ok) on `b60a149`, the `node` workspace's `missing-import` and `basic` profiles in one child
...
- **Commit workflow, decided by the owner at the end of the group 1 session (2026-09-23).** The branch must not
  ...
  (most likely with an agent) from the aggregate diff, the CHANGELOG, Architecture.md and this document, so the
  commit list is a matter of history hygiene, not of what the reviewer reads; the final merge can also be a
  squash-merge. **First act of the next session:** squash `e601aa4` through the branch head (the seven group 1 fixes and
  the two hand-off commits) into one commit on top of `1c842e6`, replace the seven hashes above with the new one,
  then push so the draft pull request's CI matrix (Ubuntu and Windows, Node 22 and 24, one job with
  `TSFLOW_ESM_HOOKS=async`) sees group 1: the new specs spawn child processes and write a fixture under
  `src/fixtures/failing/generated`, both first exercised on Linux there. Nothing was pushed in this session on
  purpose, so the squash needs no force-push.
