...
// research/local-consumer-testing.md: spawns the CLI with `--watch`, logs stdout and stderr to a file, waits for
// each "Run took" status line, writes "\n" (a rerun) until the requested number of runs is reached, then "q".
//
// usage: node research/scripts/watch-driver.js <logfile> <runs> [node args...] -- <cli args...>
//   e.g. node research/scripts/watch-driver.js research/profiles/x/dim-watch.log 2 -- -p dim
//        node research/scripts/watch-driver.js research/profiles/x/full-watch.log 2 --max-old-space-size=8192 -- -p default
// TSFLOW_WATCH_CWD sets the directory the CLI runs in (default: the UIS Tools `test` package).
...
const cwd = process.env.TSFLOW_WATCH_CWD || 'C:/Git/Azure/uis-tools/Tools.Web/VueApp/test';
