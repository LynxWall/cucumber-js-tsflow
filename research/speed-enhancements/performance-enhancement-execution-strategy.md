...
detail lives in the documents under [hand-offs/](hand-offs/) and [plan/](plan/), one per phase or stage, so that a
...
[performance-enhancement-analysis-2.md](analysis/performance-enhancement-analysis-2.md) on two independent axes, how much
performance is recovered and how hard the change is to do correctly, and executes them in phases. The worklist
is the merge of the three investigations in [analysis/](analysis/) ([performance-analysis.md](analysis/performance-analysis.md),
[analysis-2.md](analysis/analysis-2.md) and [analysis-3.md](analysis/analysis-3.md)), and the item numbers used throughout are that
worklist's numbers, unchanged. The rating scales, the summary table, every item's rating with its reasoning and
the re-rating after Phase 5 are in [ratings.md](plan/ratings.md). Items 26 to 28 were added by that
...
1. [phase-12-plan.md](plan/phase-12-plan.md) for the definition of the current stage: the stage
...
- **Release review (2026-09-25):** after 12f closed, the owner asked for an outside-in review as the maintainer
  ...
  description of 12f that follows is as it stood before the review.
...
onward were re-planned after Phase 5; see [Re-rating after Phase 5](plan/ratings.md#re-rating-after-phase-5).
...
touch the loaders once. See [Phase 1 hand-off](hand-offs/phase-01-hand-off.md) for what landed and how to use it.
...
[Phase 2 hand-off](hand-offs/phase-02-hand-off.md) for what landed and the measured effect.
...
load. See [Phase 3 hand-off](hand-offs/phase-03-hand-off.md) for what landed and the measured effect.
...
be worth far more than its rating: see [Phase 4 hand-off](hand-offs/phase-04-hand-off.md) for what landed, the
...
async `transform()` half dropped; see [Phase 5 hand-off](hand-offs/phase-05-hand-off.md).
...
picked as the phase goes. See [Phase 6 hand-off](hand-offs/phase-06-hand-off.md) for what landed, what was left, and the
...
attribution script (`research/speed-enhancements/scripts/attribute-cpuprofile.js`) are described under
[Notes specific to Phase 7](hand-offs/phase-06-hand-off.md#notes-specific-to-phase-7). Outcome: tsflow is 0.4% of `runtime:run` on both the
`dim` and the full suite, so no runtime phase follows; the full suite's warm startup is 9.1 s, of which 2.8 s is
transpile work and 5.5 s Node's module loader. See [Phase 7 hand-off](hand-offs/phase-07-hand-off.md).
...
it. Item 22 is dropped. See [Phase 8 hand-off](hand-offs/phase-08-hand-off.md) and
[`parallelLoad` removal](hand-offs/phase-08-hand-off.md#parallelload-removal-2026-09-17).
...
changed. Measured on the UIS full suite; see [Phase 9 hand-off](hand-offs/phase-09-hand-off.md).
...
process per run. See [Phase 10 hand-off](hand-offs/phase-10-hand-off.md).
...
[Phase 11 hand-off](hand-offs/phase-11-hand-off.md).
...
[Phase 12 scope](plan/phase-12-plan.md#phase-12-scope) for the strands, the decisions to take, and the exit criteria. The decisions taken at the
...
The hand-offs live under [hand-offs/](hand-offs/) and the plans under [plan/](plan/). Each begins with a link back to
this map.

| Document | Contents | Read it when |
| --- | --- | --- |
| [ratings.md](plan/ratings.md) | Rating scales, summary table, every item's rating and reasoning, what the ratings reveal, re-rating after Phase 5 (adds items 26 to 28) | An item's number, rating or original reasoning is in question |
| [phase-01-hand-off.md](hand-offs/phase-01-hand-off.md) | Items 1, 12, 13: `TSFLOW_TIMING` instrumentation and how to use it in later phases | Taking or reading a timing measurement |
| [phase-02-hand-off.md](hand-offs/phase-02-hand-off.md) | Items 2, 4, 5, 9, 11: runtime and registry hot paths; first measured effect on the large suite | Runtime lookup structures |
| [phase-03-hand-off.md](hand-offs/phase-03-hand-off.md) | Items 3, 6, 7: ESM loader and ts-node service costs, resolution cache | Loader resolution or path mapping |
| [phase-04-hand-off.md](hand-offs/phase-04-hand-off.md) | Items 8, 10, 14, 20: callsite resolution, `source-map-support` and jsdom finding, compile cache, `bindings` entry point | Startup constants, decorator-time stack walks, package `exports` |
| [phase-05-hand-off.md](hand-offs/phase-05-hand-off.md) | Items 18, 21: synchronous in-thread ESM hooks (`module.registerHooks()`), ts-node bypass | The ESM hooks model, `TSFLOW_ESM_HOOKS` |
| [phase-06-hand-off.md](hand-offs/phase-06-hand-off.md) | Items 26, 28: startup progress line, ESM callsite lines; Phase 7 measurement recipe | Startup output, themes, spinner |
| [phase-07-hand-off.md](hand-offs/phase-07-hand-off.md) | Item 27: CPU-profile attribution of the UIS runs; `default` profile startup baseline; re-rating | Where the time goes on a real suite |
| [phase-08-hand-off.md](hand-offs/phase-08-hand-off.md) | Item 16: transpile cache; `parallelLoad` A/B and removal (items 15, 17, 22 dropped); re-rating | The cache key, the preload removal |
| [phase-09-hand-off.md](hand-offs/phase-09-hand-off.md) | Items 19, 24: parse before load, selective loading and its fallbacks; re-rating | Selective loading, the module graph |
| [phase-10-hand-off.md](hand-offs/phase-10-hand-off.md) | Item 23: `--watch`, the resident process, eviction and `?tsflow=<n>` re-import; the clean reference numbers | Watch mode, the Phase 10 reference the closing measurement is compared to |
| [phase-11-hand-off.md](hand-offs/phase-11-hand-off.md) | Item 25 closed without a prototype: the bundling ceiling and feasibility findings | A future bundling attempt |
| [phase-12-plan.md](plan/phase-12-plan.md) | Phase 12 scope; baseline: decisions with the owner, coverage inventory, review findings A to AH, 12c triage, stages 12a to 12f with gates (12e includes the shipped agent skill), amended exit criteria | Starting any Phase 12 stage |
| [stage-12a-hand-off.md](hand-offs/stage-12a-hand-off.md) | Test foundation: `node:test` runner, seams, unit tests, CI matrix | The unit-test layout and seams |
| [stage-12b-hand-off.md](hand-offs/stage-12b-hand-off.md) | Behavior discovery: end-to-end specs, failure-path pass, findings Y to AH classified | A 12b finding's origin |
| [stage-12c-hand-off.md](hand-offs/stage-12c-hand-off.md) | Review refactors: groups 1 to 5 and the Z fix, the closing measurement (group 6) with finding P's compile-cache A/B, the commit workflow, the two-session working agreement, leftovers for 12d | The UIS numbers 12c closed on; why a 12c refactor is shaped as it is |
| [stage-12d-hand-off.md](hand-offs/stage-12d-hand-off.md) | Housekeeping sweeps: the strict fixes, the `typecheck` and `lint` gates, the spelling pass, the dependency audit with its open decisions, the 12c leftovers, what is left for 12e and the owner | A dependency or audit question |
...
[local-consumer-testing.md](testing/local-consumer-testing.md) (the UIS Tools
testbed and how it is linked in), [phase-4-callsite-resolution-and-jsdom.md](analysis/phase-4-callsite-resolution-and-jsdom.md)
(the Phase 4 investigation), and [Architecture.md](../../Architecture.md), which describes the product as it ships.

## Conventions

- **One hand-off document per phase or stage,** named `phase-NN-hand-off.md` or `stage-12x-hand-off.md` under
  `hand-offs/`, in the shape of the earlier ones: state of the tree, what landed, measured effect, notes specific
  to the next phase or stage. A stage that spans several sessions (12c) keeps one document and updates it as
  groups land.
...
- **Links are relative** and anchors are GitHub heading slugs. Every document starts with a link back to this
  map, and the map links to every document, so the set stays navigable from any entry point. The exception is a
  link to a source file this branch deleted, or to line numbers of a source file as an analysis read it: those
  are permalinks to `a9f7946`, the `master` commit the branch was cut from.
